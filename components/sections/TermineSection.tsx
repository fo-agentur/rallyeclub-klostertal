...
export function TermineSection({ site, termine }: { site: SiteContent; termine: TermineItem[] }) {
  return (
    <section id="termine" className="termine-zone relative scroll-mt-20 border-t border-white/10 py-20 sm:py-24 lg:py-28">
      <div className="termine-zone__glow" aria-hidden />
      <div className="termine-orb termine-orb--1 termine-parallax" aria-hidden />
      <div className="termine-orb termine-orb--2 termine-parallax" aria-hidden />
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <h2 id="headline-termine" className="font-display clip-reveal text-4xl font-bold text-white sm:text-5xl md:text-6xl">
            Termine
          </h2>
          <div className="flex flex-wrap items-center justify-end gap-3 sm:gap-4">
            <p className="font-display text-xs font-semibold uppercase tracking-[0.35em] text-rally-orange/90">{site.seasonLabel}</p>
            <a
              href="#termine-kalender"
              data-lenis-scroll
              className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-full border border-rally-accent/40 bg-rally-accent/10 px-5 py-2.5 text-sm font-semibold text-rally-orange shadow-[0_0_24px_-8px_rgba(249,115,22,0.45)] transition hover:border-rally-orange hover:bg-rally-accent/20 hover:text-white"
            >
        ...
        <p id="termine-intro" className="mt-6 max-w-3xl text-lg leading-relaxed text-rally-muted">
          ...
          <a
            className="text-rally-orange underline decoration-white/20 underline-offset-4 hover:text-white"
        ...
        <div className="termine-overview mt-10 px-4 py-5 sm:px-6" role="region" aria-label="Termine Kurzübersicht">
          <p className="font-display text-[10px] font-bold uppercase tracking-[0.4em] text-rally-orange/90">Auf einen Blick</p>
                ...
                <span
                  className={
                    card.badgeTone === 'accent'
                      ? 'mt-2 inline-flex rounded-full border border-rally-accent/35 bg-rally-accent/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-rally-orange'
                    ...
                    <span
                      className={
                        (t.category ?? '').toLowerCase().includes('slalom')
                          ? 'inline-flex rounded-full border border-rally-accent/40 bg-rally-accent/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-rally-orange'
