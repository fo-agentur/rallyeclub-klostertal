        ...
        <div className="container-wide">
          ...
          {leadPost ? (
            <div className="grid gap-6 lg:grid-cols-[2fr_1fr_1fr]">
              {/* Lead card */}
              <Reveal variant="up">
                <Link href={`/news/${leadPost.slug}`} className="group relative block cursor-pointer">
                  <div className="relative aspect-[16/10] overflow-hidden bg-ink">
                    {leadPost.cover_image ? (
                      <Image
                        ...
                        alt={leadPost.title}
                        fill
                        className="object-cover transition duration-[700ms] ease-out group-hover:scale-[1.06]"
                    ...
                    {/* Hover shine sweep */}
                    <div className="pointer-events-none absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/10 to-transparent transition-transform duration-[900ms] ease-out group-hover:translate-x-full motion-reduce:hidden" />
                    <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
                      ...
                      <h3 className="mt-3 font-display text-[40px] uppercase leading-[0.95] transition-colors group-hover:text-racing">
                        {leadPost.title}
                      </h3>
                      {leadPost.excerpt && (
              ...
              {smallPosts.map((post, idx) => (
                <Reveal key={post.id} variant="up" delay={120 + idx * 100}>
                  <Link href={`/news/${post.slug}`} className="group block cursor-pointer">
                    ...
                    <h3 className="mt-2 font-display text-2xl uppercase leading-tight transition-colors group-hover:text-racing">
                      {post.title}
                    </h3>
                    {post.excerpt && (
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {[
                ...
                <Reveal key={item.title} variant="up" delay={i * 90}>
                  <Link href={item.href} className="card-hover group block rounded-[24px] border border-neutral-200 bg-neutral-50 p-6 hover:border-racing/40 hover:bg-white hover:shadow-card">
                    <div className="text-[11px] font-bold uppercase tracking-[0.22em] text-racing">RCK</div>
                    <h3 className="mt-3 font-display text-2xl uppercase leading-tight text-ink transition-colors group-hover:text-racing">
                      {item.title}
                    </h3>
                    <p className="mt-3 text-sm leading-7 text-neutral-600">{item.text}</p>
                    <span className="mt-4 inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-[0.18em] text-neutral-400 transition-all group-hover:gap-2 group-hover:text-racing">
          ...
          <div className="grid gap-12 lg:grid-cols-[3fr_2fr] lg:gap-16">
            ...
            <Reveal variant="left">
              <div>
                  ...
                  {albums.length > 0
                    ? albums.map((album, i) => (
                        ...
                        >
                          ...
                          {album.title && (
                            <span className="absolute bottom-2 left-2 right-2 translate-y-2 text-[10px] font-bold uppercase tracking-[0.16em] text-white opacity-0 transition duration-500 group-hover:translate-y-0 group-hover:opacity-100">
                              {album.title}
                            </span>
                          )}
                        </Link>
                ...
                <Link href="/galerie" className="btn-ghost mt-6 inline-flex link-grow">
                  Komplette Galerie →
                </Link>
              </div>
            </Reveal>
              ...
              <div>
                ...
                <div className="mt-8 flex flex-col gap-4 text-sm">
                  {[
                    ...
                    <div key={row.lbl} className="group flex gap-4 transition-colors hover:text-white">
                      <span className="w-24 shrink-0 text-[11px] font-semibold uppercase tracking-[0.18em] text-neutral-500 transition-colors group-hover:text-racing">
                        {row.lbl}
                      </span>
                      <span className="text-neutral-300 transition-colors group-hover:text-white">{row.val}</span>
                    </div>
                  ))}
                </div>
        ...
        <div className="container-wide">
          <Reveal>
            <div className="flex flex-col items-center gap-8">
              ...
              <SponsorStrip sponsors={sponsors} marquee />
            </div>
          </Reveal>
        </div>
      </section>
